...
class Engine(object):
    ...
    def main(self):
        ...
        elif self.option.model_type == 'swin':
            swin_model = build_model(self.config)
            hash_model = Swin4Hash(self.option, swin_model)

        if self.option.center_model=='center':
            center_model = CenterModel(self.option)
        elif self.option.center_model=='dual':
            center_model = DualCNN_GNNCenterModel(self.option)
            self.state['part_loss_flag'] = True
            self.state['part_loss'] = 0.0
        else:
    ...
    def run_epoch(self, hash_model, center_model, criterion, criterion_center, optimizer_hash, optimizer_center,
        ...
        for epoch in range(start_epoch + 1, self.option.epochs):
            ...
            '''
            if 'part_loss_flag' in self.state.keys():
                hashCenter_pre, word_embedding, self.state['part_loss'] = \
                    self.forward_hashCenter(center_model, train_loader)
            else:
...
if __name__ == '__main__':
    ...
    option, config = options.par_option()
    Logger.divider("print option")
    for k, v in vars(option).items():
        Logger.info('\t{}: {}'.format(k, v))
    state = {'start_time': start_time}
    engine = Engine(option=option, state=state, config=config)
    engine.main()
